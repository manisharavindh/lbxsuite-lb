...
const BlogPage = () => {
  ...
  useEffect(() => {
    if (!loading) {
      const queryParams = new URLSearchParams(location.search);
      if (queryParams.get('tag') || location.hash === '#filter-bar') {
        setTimeout(() => {
          const filterEl = document.getElementById('filter-bar');
          if (filterEl) {
            const y = filterEl.getBoundingClientRect().top + window.scrollY - 72;
      ...
      <main className="relative z-10 w-full flex flex-col items-center">
        {loading ? (
          ...
          <>
            ...
            {/* ============ FILTER BAR ============ */}
            <section id="filter-bar" className="w-full bg-[#141414] border-y border-white/5 sticky top-[72px] z-30 backdrop-blur-xl bg-[#141414]/90">
